...
function plg_projectdata($data, $lang, $o) {
    ...
    if ($data["type"] == "sum") {
        ...
        $sdata = $lib->db->get_row("com_project", "*");
        $r .= "<div class='projectdata '>";
        $r .= "<div class='sum'>";

        $r .= "<div class='projectes'><span>" . $lang['project'] . " " . count($sdata) . "</span> " . $lang['allProjects'] . " </div>";
        $r .= "<div class='donations'><span>" . $lang['cun'] . " " . $umpric['price'] . "</span>  " . $lang['alldonations'] . " </div>";
        ...
        $umpric = $lib->db->get_row("plugincart_order_item", "sum(price) as price", "item_id=" . $data["id"]);
        ...
        if ($data["type"] == "title" || $data["type"] == "all") {
            $r .= "<div class='plg_prodataTitle'>" . $lib->language->getText("com_project", "title", $sdata['id'], $sdata['title'])  . "</div>";
        }
        // </editor-fold>
...
// </editor-fold>
        ...
        if ($data["type"] == "users" || $data["type"] == "all") {
                        $all = $lib->db->get_data("plugincart_order_item", "*", "item_id=" . $data["id"]);
//
