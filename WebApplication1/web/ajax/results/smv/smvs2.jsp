...
<script>
    $(function() {
        ...
        function getmydata(id) {
            ...
            ajaxAbort(x2);
            defajax = loadAjax("ajax/results/smv/smvBody.jsp?id=" + id + "&point=<%=request.getParameter("point")%>&res_id=<%=request.getParameter("res_id")%>&temp=<%=request.getParameter("template")%>&state=<%=request.getParameter("state")%>", ".smv_body2", "all,main");
        }

        $(".select_ch2").change(function() {
            var id = $(this).val();
            getmydata(id);
        })

        var id = $(".select_ch2").val();
        getmydata(id);
...
<div class="smv">
    ...
    <div class="smv_top">
        ...
        <div><label for="select_ch2"></label>
            <select class="select_ch2" name="select_ch2"> 
                <%
