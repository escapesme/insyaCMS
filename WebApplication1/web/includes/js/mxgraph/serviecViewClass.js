...
function makGService(cont, model, s_id, mod_id, pro_id, locked, canedit) {
    ...
    gra.setTooltips(true);




    gra.panningHandler.selectOnPopup = false;
    gra.setAutoSizeCells(false);
    if (mxClient.IS_IE)
    {
...
function mack_ServiceEvents(G, s_id, mod_id) {
    ...
    G.addListener(mxEvent.CELLS_FOLDED, function(sender, evt) {
        ...
        if (type == true) {
            ...
            cell[0].setStyle(cell[0].getStyle() + ";shape=image; ;verticalAlign=top ;verticalLabelPosition=middle;fontColor=#0000000");
        } else {
                ...
                if (myarr1[i] != "") {
                    ...
                    if (myretdata1[0] != "" && myretdata1[0] != null) {
                        ...
                        if (myArray.lastIndexOf(my_id1) == -1) {
                            ...
                            var hei = 130;
                            //draw the node             
                            //  alert ('G.insertVertex(cell,'+myid22+','+my_name1+', 100,100,'+wid+','+hei+', "st"+tag+"10000;verticalAlign=top ;verticalLabelPosition=middle;")');
                            window["l_node11" + my_id1] = G.insertVertex(cell, myid22, my_name1, 100, 100, wid, hei, "st" + tag + my_id1 + "10000;verticalAlign=top ;verticalLabelPosition=middle;");
                            //  alert (id_p);
                            ...
                            if (id_p == "in") {
                                ...
                                window["l_node11" + my_id1 + "ch"] = G.insertVertex(window["l_node11" + my_id1], myid22, my_name1, 100, 100, wid, hei, "st10000;");
                                myAn[0] = window["l_node11" + my_id1];
                                G.foldCells(true, true, myAn);
                                var geo = G.getModel().getGeometry(window["l_node11" + my_id1]);
                                window["l_node11" + my_id1].setStyle(window["l_node11" + my_id1].getStyle() + "verticalAlign=top ;verticalLabelPosition=middle;");
                                geo.width = wid;
                                geo.height = hei;
                    ...
                    $.get("ajax/views/serviecView/getData.jsp?" + mylink, function(p_data) {
                        ...
                        $.get("ajax/views/serviecView/getData.jsp?" + mylink, function(s_data) {
                            ...
                            if ($.trim(s_data[1]) !== $.trim("collector")) {
                                ...
                                newProcess(g, cell[0].source, cell[0].target, s_id, Mod_id, pro_id, p_data, s_data, s_data[1], cell[0].source.parent.getId());
                    ...
                    if (cell[0].source.parent.getId() == cell[0].target.parent.getId()) {
                        ...
                        $.get("ajax/views/serviecView/getData.jsp?" + mylink, function(s_data) {
                            ...
                            if ($.trim(nodeData[1]) !== $.trim("insertion")) {
                                ...
                                if (("<div class='texth'>End</div>" == t.getValue() || t.getValue() == "End") && t.getId().split("/")[1] != null) {
                                    var mylink = "MODEL_ID=" + Mod_id + "&status=addLine&sourceId=" + cell[0].source.getId() + "&tid=" + t.getId() + "&uF=100";
                                    $.get("ajax/views/serviecView/createNode.jsp?" + mylink, function(r_data) {
                                        closeAllWindows();
                                        muUpdat(cell[0].source.parent, g, s_id, Mod_id, pro_id);
                                    })

                                } else {
                                    if (("<div title='Start' class='tooltip texth'>Start</div>" == cell[0].source.getValue() || cell[0].source.getValue() == "Start") && $.trim(cell[0].source.parent.getId()) == "1" && $.trim(s_data.split(",")[1]) !== "process") {
                                        alert("Cannot create this line! The start node of the first level may only connect with a process");
                                        ...
                                        makeWinDows("Create Service Node", "newProcess", newProcessline(g, cell[0].source, t, s_id, Mod_id, pro_id, cell[0], s_data[1], cell[0].source.parent.getId()), 300, 3, 550, null);
                                    }
                                }
                            } else {
...
function creatDService(G, parent, id, mytext, x, y, height, width, style_number, mstyle, layer0, image) {
    ...
    var myr = G.insertVertex(parent, id, "<div title='" + mytext + "' class='tooltip texth'>" + mytext + "</div>", x, y, width, height, "st" + style_number + ";labelPosition=center;lineHight=1.2;overflow=fill;" + moreStyle + ";verticalAlign=top ;verticalLabelPosition=middle;fontColor=#0000000;fontSize=12"); // G.insertVertex(parent, null, mytext , x, y, width,height,"st"+style_number+shapetype );// mxGraph.insertVertex(parent, id, value, x, y, width, height, style)


    if (G.tooltipHandler.state == G.view.getState(myr))
    {
        G.tooltipHandler.div.innerHTML = 'hello';
    }
