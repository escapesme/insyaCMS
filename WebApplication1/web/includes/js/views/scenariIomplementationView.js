...
function showPointAddressbayl(longt, lat, id, image, width, height, mp, name, x, y, mid, sid, status, clouddata) {
    ...
    google.maps.event.addListener(marker, "dblclick", function() {
        ...
        $.get("ajax/implementation/ImplementationGroups.jsp?sid=" + sid + "&id=" + daa[1] + "/" + daa[2] + "/" + daa[3], function(retdata_group) {
            ...
            var data = "<div class='mydata'> " + $.trim(retdata_group) + "</div>" + "<script>newGroupActions('" + marker.id + "','" + daa[3] + "');windowsCancelDo(\".cancel_bt\" ,\"" + marker.id + "\");\n\
</script><div class='do_center'> \n\
<input type=\"button\" class=\"cancel_bt\" value=\"Cancel\"  /></div>"
