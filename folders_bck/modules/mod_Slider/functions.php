...
function mod_sliderData_Block($pro, $lang, $tableName, $tablecatName, $data, $caturl) {
    ...
    if ($pro['linkType'] == "more") {
        $mydata .= '<a class=\'link\' href=\'' . $url . '\'>' . $lang['readMore'] . '</a>';
    }
...
<script>
    ...
    if ($pro['buttonsShow'] == "1") {
        $returnData.=' 
					prev: \'#multiple_slides_visible' . $myid . ' .prev\',
					next: \'#multiple_slides_visible' . $myid . ' .next\',';
    }
    if ($pro['pagesShow'] == "1") {
        $returnData.=' 
                                        pagination: "#multiple_slides_visible' . $myid . ' .pager",';
    }
