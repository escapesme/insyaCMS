...
function pro_mod_sourec($l, $p) {
    ...
    foreach ($all as $a) {
        ...
        if ($a['url'] != "") {
            $data .= "<div > <a  href='/do/" . $a['id'] . "/res/ '  class='lightbox-url'>" . $l['gomore'] . "</a></div></div>";
        }
    }
    $data.="</div>";
    $r = '<div class="pro_model">' . $data . '</div>';
...
$all=$all = $lib->coms->project-> PayProjectdata($id,"*"," limit 0,4");
...
//$u['gender']
        ...
        $data .="<div class='image'><img src='/uploads/images/" . $image . "' /></div></div>";
    }
    ...
    if (is_array($all) && count($all) > 0) {
        ...
        $r = '<div class="pro_model"><div class="title">' . $lng['users'] . '</div>' . $data . '</div>';
    }
...
function com_project_getPrices($p, $l, $id) {
    ...
    $r.="<div class='price more'>"
            ...
            . "" . $l["moremsg1"] . "<input type='text'  data-min='" . $l["MoreMinimumDonation"] . "' class='moreprice' value='" . $l["MoreMinimumDonation"] . "' />" . $l["moremsg2"] . ""
            . "</div>"
            . "<div class='parrow'></div></div>";
