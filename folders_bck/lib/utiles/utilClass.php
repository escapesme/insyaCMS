...
//echo $this->variables->url_view ;
    ...
    function readXmlAttributes($file, $tag) {
        $xml = simplexml_load_file($file);
...
// }
    ...
    function importfiles($link, $file = "") {
        ...
        if (is_file($link . "/" . $file . "script.js")) {
          ...
          //  echo "<script src= \"/" . $link . "/" . $file . "script.js\" ></script>";
    ...
    function sendHTMLMail($emails, $HTMLID, $subject, $formlabel, $formmail, $dataVars) {
      /* @var $lib  \libs\libs */
    global $lib;

        $message = $this->db->get_row("com_html", "*", "id=" . $HTMLID);

        $r = $lib->util->data->TextVarUpdate($dataVars, $message['my_body']);
