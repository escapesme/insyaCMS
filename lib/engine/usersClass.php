...
//put your code here
    ...
    function dologin($loction) {
        ...
        if (isset($_POST[login_email]) && $_POST["dologin"]) {
            $login = $this->db->get_row('com_users', '*', " email='" . trim($_POST[login_email]) . "' and  password=MD5('" . trim($_POST[password]) . "') ");
            if (is_array($login)) {
                if (($login['enabled']) != 0) {
                    ...
                    echo '<script>window.location.href = "' . $lang . $loction . '";</script>';
                    $status = "login";
    ...
    function checklogin($mail, $pass) {
        ...
        $login = $this->db->get_row('com_users', '*', " email='" . trim($mail) . "' and  password=MD5('" . trim($pass) . "') ");
    ...
    function forgotpassword($mail) {
        ...
        $login = $this->db->get_row('com_users', '*', " email='" . trim($mail) . "'");
        ...
        if (is_array($login)) {
            ...
            $this->db->update_data('com_users', $ffl, " email='" . trim($mail) . "'");
            $r = $newpassword;
        } else {
            $r = false;
        }
...
';
    ...
    function loginUsers($id, $reloadTo = "") {
        ...
        if ($reloadTo != "") {
            ...
            echo '<script>'
            . 'window.location.href = "' . $lang . $reloadTo . '";'
            . '</script>';
    }}
/**
 * $minData[name,user_name,email,enabled,password,g_id,....]
 * @return user ifo Row [name,user_name,email,enabled,password,g_id,....]
 */
    function getAddUser($data) {
        $this->db->insert_row('com_users', $data);
        return $this->db->get_maxrow('com_users');
    }
