...
/*
 ...
  */
function advuploader(selector, onComplete, itemLimit) {
    if (!itemLimit) {
        itemLimit = 5;
    }
...
function cropdata(img) {
    $(".pic_preview img").attr("src", "/uploads/images/" + img);
    $("." + cropName + ".imageInput").append('<input type="hidden" name="th_image" value="' + img + '">');
    $(".imagetitle").text("" + img);
}
...
$(function () {
    ...
    $(".crop-btn").click(function () {
        var src = $(".pic_preview img").attr("src");
        cropName = $(this).data("name");
        $.colorbox({html: "<div class='box'><iframe src=\"/ajax/admin/crop.php?img=" + src + "\"></iframe>" + "</div>", width: "80%", height: "80%"});
        return false;
...
function setFormToSesstion(Form, callback, name) {
    ...
    $(function () {
        ...
        if (Form.data("saveto") != null) {
            name = Form.data("saveto");
        }
