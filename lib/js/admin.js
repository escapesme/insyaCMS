...
function updatemain(more) {

    var info = $(".pageinfo").val();
    if (more) {
        info += more;
    }
    var a=getadminAjax("admin/update_list", info, function(retdata) {
        $(".mainpage").html(retdata);
...
//$("#multiform").submit(); //Submit the form
    ...
    /* $(document).on("click", " .addbt,.ajax.tabsstatus .edit_option,.ajax.tabsstatus .group_bar a", function() {
     ...
     $(".page-content").append("<div style='display: block;' data-info='" + info + "'  data-id='" + id + "'  data-parentlistid='" + actTab + "'  class=\" ajax tabsstatus tabsbody " + id + "\">" + retdata + "</div>");
