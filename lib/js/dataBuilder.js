...
function builder_getTypehtmlData(order, id, optionsData, type, name) {
    ...
    var setpdiv = "<div class='chstep'>\n\
               ...
               <div class='stepNumer'>Type</div>\n\
                <li title='Collapse All Children'  class='closeChtypeBt '>\n\<i class=\"fa fa-trash-o\"></i> </li>\n\
               </div>\n\
              <div class='chstepOpions'><input type='hidden' class='mydata' value='" + optionsData + "'/></div><div class='my_clearfix' > </div>\n\
            </div>";
    return setpdiv;
}
...
function builder_getStephtmlData(order, id, title, TypesData) {
    ...
    return  "<div class='step'>\n\
                       ...
                       <div class='stepOrder'>" + order + "</div>\n\
                        ...
                        <li title='Delete'  class='closeChStepBt'>\n\<i class=\"fa fa-trash-o\"></i></li>\n\
...
function updateAfterAjax(file) {
    ...
    $("body").on("cu", ".stepes", function () {
        ...
        $(".chstepOpions").each(function (index) {
            ...
            getAjaxPost(file, "sumber=" + index + "&type=" + data['type'] + "&id=" + data['typeID'] + "&data=" + mychmypro + more, function (d) {
                $(obj).html(d);
                if (index == len - 1) {
                    $(".pluginTitle").each(function (index2) {
                        ...
                        getAjaxPost("admin/pluginoption", "sumber=" + index2 + "&plugin=" + $(this).val() + more, function (d) {
                            i++;
...
function Bluder_update() {
    ...
    getAjaxPost("admin/urlcode", "data=" + tsdata + "&status=en", function (d) {
...
//"admin/wizard_data"
    ...
    function getmyajax(type, obj) {
        ////
        getAjaxPost(file, "type=" + type, function (d) {
    ...
    $(document).on('click', myclass + " .stepHeader", function () {
        $(".chstepOpions").hide();
        $(this).parent().find(".chstepOpions").show();

        $(myclass + " .stepHeader").removeClass("act");
        $(this).addClass("act");
        //  alert("dasdl;as");
    ...
    $(document).on('change', "#queryid", function () {
        ...
        getAjaxPost(file, "type=" + type + "&" + $(this).attr("id") + "=" + $(this).val(), function (d) {
